...
from lib.CAModel import NoGCAModel
...
pad_target = torch.from_numpy(pad_target.astype(np.float32)).to(device).type(torch.float32)

seed = make_seed((h, w), CHANNEL_N)
pool = SamplePool(x=np.repeat(seed[None, ...], POOL_SIZE, 0))

model = NoGCAModel(CHANNEL_N, CELL_FIRE_RATE, device)
